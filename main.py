            ...
            if retorno.ok:
                if cidades:
                    ...
                    if scores:
                        ...
                        if save:
                            workbook = xlsxwriter.Workbook(f'{save}')
                            ...
                            nome.configure(text="O arquivo não foi salvo.",
                                           foreground="#ff1010", justify=alinhamento)
                    else:
                        nome.configure(text="Nenhuma empresa encontrada.",
                                       foreground="#ff1010", justify=alinhamento)
                    scores.clear()
                else:
...
# Select City
limoeiro = ttk.Checkbutton(frameSide, text="Limoeiro de Anadia", command=lambda: marcar(limoeiro, "LIMOEIRO DE ANADIA"), style="success")
limoeiro.pack(pady=(0, 10), padx=5, fill="both")
limoeiro.invoke()
limoeiro.invoke()
giral = ttk.Checkbutton(frameSide, text="Giral do Ponciano", command=lambda: marcar(giral, "GIRAL DO PONCIANO"), style="success")
giral.pack(pady=(0, 10), padx=5, fill="both")
giral.invoke()
giral.invoke()
taquarana = ttk.Checkbutton(frameSide, text="Taquarana", command=lambda: marcar(taquarana, "TAQUARANA"), style="success")
taquarana.pack(pady=(0, 10), padx=5, fill="both")
taquarana.invoke()
taquarana.invoke()
arapiraca = ttk.Checkbutton(frameSide, text="Arapiraca", command=lambda: marcar(arapiraca, "ARAPIRACA"), style="success")
...
browser = ttk.Radiobutton(frameRadials, text="Firefox", variable=bw, command=lambda: select(0), value=0, style="success")
browser.pack(side=tk.LEFT, padx=(5, 0))
browser.invoke()
browser2 = ttk.Radiobutton(frameRadials, text="Chrome", variable=bw, command=lambda: select(1), value=1, style="success")
